...
with open('GrammarofTime/gots_dictionary.json') as data_file:
            ...
            if not isfloat(val):
                if val is "":
                    ...
                    if operands is "":
                        ...
                        ns[i] = eval(
                            gots_func_dict["pre_processing"][operator] + '(ns[' + str(i) + '],' + operands[:-1] + ')')
                    operands = ""
            else:
                continue
            ...
            if not isfloat(val):
                if val is "":
                    ...
                    if operands is "":
                        ...
                        _constr = eval(
                            gots_func_dict["connotation"][operator] + '(ns[' + str(i) + '],' + operands[:-1] + ')')
...
def open_files():  # opens the file collection and creates de dataset to be used with var X, Y and Time
    MouseX = []
    MouseY = []
    MouseTime = []
...
def VelocityinX(data):
    time_var, space_var = interpolate_data(data,t_abandon=20)
    return time_var['vx']

def VelocityinY(data):
    time_var, space_var = interpolate_data(data, t_abandon=20)
    return time_var['vy']
